...
# dictionar[x] contine toate caracterele care sunt xorate cu elementul de pe pozitia x din parola

dict = {}
for x in range(lungime_cheie):
    dict[x] = set()
    for y in range(x,len(lista_bytes),lungime_cheie):
        dict[x].add(chr(int(lista_bytes[y],2)))


# l contine toate caracterele care s-ar putea gasi in parola
